<div class="content_block">
    <h1>{$lang['viewing_news']}</h1>
    {if $error}<div class="critical">{$error}</div>{/if}
    {if $alert}<div class="alert">{$alert}</div>{/if}
    <div class="groupbox"><div style="float:right">{$n['date_human']}</div>{$n['title']}</div>
    <div class="row1 post">
        {$n['content']}
    </div>
    ...
    <div class="subbox{if $c['staff']}staff{/if}">
        <div class="links" style="float:right">{$c['time_ago']}{if $c['can_edit']}&nbsp;&nbsp;<a href="{$td_url}/index.php?page=news&amp;act=editcomment&amp;id={$c['id']}"><img src="{$img_url}/edit_icon.gif" alt="{$lang['edit']}" id="edit_{$c['id']}" style="vertical-align: middle" /></a>{/if}{if $c['can_delete']}&nbsp;&nbsp;<a href="{$td_url}/index.php?page=news&amp;act=dodeletecomment&amp;id={$c['id']}" onclick="return sure_delete()"><img src="{$img_url}/delete_icon.gif" alt="{$lang['delete']}" id="delete_{$c['id']}" style="vertical-align: middle" /></a>{/if}
    ...
    <form action="{$td_url}/index.php?page=news&amp;act=doaddcomment&amp;id={$n['id']}" method="post">
